...
class _ZegoAudioPageState extends State<ZegoAudioPage>
  ...
  final ValueNotifier<bool> isStartedNotifier = ValueNotifier(false);
          ...
          child: ValueListenableBuilder<bool>(
            ...
            builder: (context, isLoading, child) {
              return ElevatedButton(
                onPressed: isLoading ? null : logoutRoom,
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color.fromRGBO(230, 102, 102, 1),
  ...
  Future<void> startAudioChat() async {
    ...
    isStartedNotifier.value = false;
    try {
      final success = await aiAgentService.startCall();
      isStartedNotifier.value = success;
      if (!success && mounted) {
  ...
  Future<void> logoutRoom() async {
    if (!isStartedNotifier.value) {
      /// start call失败了，直接返回上一页
      if (mounted) {
